...
static int is_video_decoder(int fd, const char *name)
{
	...
	if (!(cap.capabilities & V4L2_CAP_STREAMING) ||
		...
		dbg("Insufficient capabilities for video device (is %s correct?)",
		    name);
		return -1;
	}
...
int video_g_fmt(int fd, unsigned int *width, unsigned int *height)
{
	...
	ret = ioctl(fd, VIDIOC_G_FMT, &g_fmt);
	if (ret) {
...
int video_dequeue_output(struct instance *i, int *n, unsigned int *buf_flags)
{
	struct v4l2_buffer buf;
...
int video_setup_capture(struct instance *i, int num_buffers, int w, int h)
{
	...
	info("  %dx%d fmt=%s (%d planes) field=%s cspace=%s flags=%08x",
